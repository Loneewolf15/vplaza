...
const page = () => {
  ...
  // Handle adding image from "Click to add a picture"
  const handleAddImageClick = () => {
    fileInputRef.current.click();
  };

  // Handle adding image from "+ icon" button
  ...
  const handleSave = async () => {
    ...
    try {
      // Upload images to Sanity.io and collect URLs
      const imageUrls = await Promise.all(
        imageFiles.map((file) => uploadImageToSanity(file))
      );

      // Proceed with API call only after all images are uploaded
  ...
  return (
    <div className="relative flex flex-col items-center p-6 bg-[#D9D9D9] rounded-lg shadow-lg w-full min-h-screen">
      ...
      <div
        className="bg-[#004AAD] w-full text-center font-bold text-lg rounded-lg text-white py-2 mt-2"
        onClick={handleSave}
      >
        Add
      </div>
